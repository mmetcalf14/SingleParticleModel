...
void CorrMat::BuildCorrMat(const Hamiltonian & H)
{
    ...
    MatrixXd Mat = MatrixXd::Zero(Nsite, Nsite);
    for(int k = 0; k < Np; k++){
      VectorXd vec = H.EVec.col(k);
      Mat += vec * vec.transpose();
    }
    C_mat = Mat.cast <complex<double> > ();
//    cout << "C_mat initial: \n" << C_mat(0,0).real() << endl;
...
void CorrMat::RKAdiabatic(Hamiltonian & ham, ofstream &fout, ofstream &Fout,
  const double dt, const double N_it, const double T, const double J_0,
  const double delta_0, const double h_0)
{
    MatrixXcd k1(Nsite, Nsite);
    MatrixXcd k2(Nsite, Nsite);
    MatrixXcd k3(Nsite, Nsite);
